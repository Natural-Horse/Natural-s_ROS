...
vector<pff> door_center(30);                                        //找门阶段找到的门的中心
pff door_center_ENU;
...
int main(int argc, char** argv)
{
    ...
    for(int i = 1; i <= 20 && ros::ok(); i++){
        ...
        // cout << "开始找门" << endl;
        cout << "找到第" << i << "个门; ";
        door_center[++cnt] = find_door_center();
    ...
    //把找出来的门中心转换到ENU坐标系
    // cout << "找到的门的中心FLU：x: " << door_center_x << " y: " << door_center_y << endl;
    ...
    cout << "找到的门的中心ENU：x: " << door_center_x << " y: " << door_center_y << endl;
    //printf();
    // return 0;
...
pff find_door_center(){
    ...
    for(int i = 0; i <= 359; i++){
        ...
        pos_x[i] = Laser.ranges[i] * cos((float)i / 180 * M_PI);
        pos_y[i] = Laser.ranges[i] * sin((float)i / 180 * M_PI);
        // cout << "pos_x : " << pos_x[i];
        // cout << "pos_y : " << pos_y[i] << endl;
    }
    ...
    for(int i = 1; i <= 89; i++){
        if(check_vertical(i, false) == false){
            ...
            if(last_vertical_flag == 1){
                ...
                cout << "角度为" << i - 1 << "时发现" << cnt << "号门右门框";
                cout << " check_vertical = " << "false" << endl;
            }
        ...
        if(check_vertical(i, true) == false){
            ...
            if(last_vertical_flag == 0){     
                if(flag_2 == 0 && !center_flag){
                    ...
                    flag_2 = 1;
                    cout << "角度为" << i << "时发现" << "1号门右门框" ;
                    cout << " check_vertical = " << "true" << endl;
                }
                else{
                     possible_door_right_edge[cnt] = i;
                    cout << "角度为" << i << "时发现" << cnt << "号门右门框";
                    cout << " check_vertical = " << "true" << endl;
                }               
            }
            last_vertical_flag = 1;
...
pff change_FLU_to_ENU(float x, float y){
    ...
    // cout << "target_angle_FLU: " << target_angle_FLU << endl;
    float target_angle_ENU = target_angle_FLU + yaw_angle / 180 * M_PI;
    // cout << "yaw_angle : " << yaw_angle << endl;
    // cout << "target_angle_ENU : " << target_angle_ENU << endl;
    float target_x_ENU = target_distance * cos(target_angle_ENU) + pos_drone.pose.position.x;
    float target_y_ENU = target_distance * sin(target_angle_ENU) + pos_drone.pose.position.y;
